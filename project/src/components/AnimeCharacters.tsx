...
export default function AnimeCharacters({ setCharactersView }: AnimeCharactersProps) {
  const navigate = useNavigate();

  return (
    <div className="max-w-7xl mx-auto px-4 py-16">

        {/* Botón triangular para volver al hub */}
        <motion.div
        ...
        <button onClick={() => setCharactersView("hub")} className="cyberpunk-button-back cyberpunk-heading group flex items-center space-x-2 mt-4 mb-8">Back</button>
        <h1 className="cyberpunk-heading text-4xl font-bold text-cyber-primary-300 mb-8">Anime Characters</h1>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {animeCharacters.map((character: Character) => (
            <div
                key={character.name}
