...
export default function SelectedCharacter() {
  ...
  return (
    <motion.div
      ...
      className="max-w-4xl mx-auto px-4 py-16 text-white bg-cyber-bg-900 rounded-lg shadow-lg"
    >
      {/* Botón de regreso */}
      <motion.button
        className="cyberpunk-button-back cyberpunk-heading group flex items-center space-x-2 mt-4 mb-8"
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
        ...
        <motion.img
          ...
          alt={character.name}
          className="w-64 h-64 object-cover border-4 border-cyber-yellow shadow-lg"
        ...
        <div>
          <h1 className="text-4xl font-bold text-cyber-secondary-500">{character.name}</h1>
          <p className="text-lg text-gray-300 mt-4">{character.description}</p>
        </div>
      </div>
    </motion.div>
  );
